...
export default function Today() {
  ...
  const currentUser = useSelector(((state) => state.auth.user))
    ...
    useEffect(() => {
      fetch(`http://localhost:3000/users/${currentUser.id}/appointments/by_date/${new Date().toISOString().slice(0, 10)}`, {
        headers: {
    ...
    <div className="py-4 px-4 lg:px-64 w-full">
      ...
      {appointments.map((item) => (
        <div key={item.id}>
          <CardMeeting
            data={{
              upLeft: item.datetime.split('T').pop().split('.').shift().slice(0, -3),
